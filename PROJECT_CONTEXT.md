...
- 適用範圍：12 大系統、58 個子系統；一般頁面由 `web/components/SystemPageHeader.tsx`
...
- 58 個子系統圖卡標準：桌面瀏覽器 100% 固定 269×200px；一般頁首沿用距頂列 22px、
...
- 實作集中於 `web/app/button-standard.css`，由根版面 `web/app/layout.tsx` 最後載入，故 12 大系統的
  58 個子系統共用同一份規格，不需逐頁複製 CSS。
...
- **V2 全站表頭功能按鈕由共用元件管理**：12 大系統／53 個子系統的一般內容頁頂列只保留「首頁」「個人資料」「登出」；戰情儀表板、維修／派完工、駐衛警巡檢、電子交接簿與後台等系統切換按鈕移至入口頁、系統頁及後台側欄，不在頂列重複顯示。首頁使用 `assets/system-icons/home-nav-icon.png`，個人資料使用 `assets/system-icons/profile-nav-icon.png`，兩者為同一套藍／青色立體生圖 ICON；登出維持文字按鈕。三個全螢幕圖資工具頁保留必要的圖資工作連結，視為工具列例外。這是業主明確指定的現行標準。

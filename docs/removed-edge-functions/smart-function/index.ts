...
Deno.serve(async (req) => {
  ...
  try {
    ...
    else if (payload.table === "inspection_records" && payload.record?.run_status === "abnormal") {
      if (s.line_notify_inspect !== "true") return json({ ok: true, msg: "disabled" });
      const rec = payload.record;
      let eqName: string = rec.equipment_id ?? "—";
      if (rec.equipment_id) {
        const { data: eq } = await db.from("equipment").select("name").eq("equipment_id", rec.equipment_id).single();
        if (eq?.name) eqName = eq.name;
      }
      const dtStr = new Date(rec.inspect_time ?? rec.created_at).toLocaleString("zh-TW", { timeZone: "Asia/Taipei" });
      text = `⚠️ 巡檢異常警報\n設備：${eqName}\n時間：${dtStr}\n說明：${rec.note ?? "（未填）"}`;
    }

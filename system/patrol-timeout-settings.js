(function(){
  ...
  function ruleHtml(r,i){
    return `<details data-rule="${i}" ${r._open?'open':''} style="margin-bottom:10px;border:1px solid rgba(180,138,255,.25);border-radius:4px;background:rgba(180,138,255,.04)">
      <summary style="display:flex;align-items:center;gap:12px;padding:12px;cursor:pointer;list-style:none;font-weight:700;color:var(--text-hi)"><span style="color:#b48aff">▸</span><span data-summary-label>${esc(r.label||'未命名時段')}</span><span data-summary-time style="color:var(--text-dim);font-weight:400">${esc(r.start)}～${esc(r.end)}</span><span style="margin-left:auto;color:${r.enabled!==false?'var(--green)':'var(--text-dim)'}">${r.enabled!==false?'啟用':'停用'}</span></summary>

...
## 介面語系規格：繁體中文（2026-08-19）

V2 的 8 大系統、48 個子系統皆以臺灣繁體中文作為主要顯示語言，後續新增頁面與元件必須遵循：

1. 頁面標題、導覽、欄位標籤、按鈕、提示、錯誤訊息、空狀態、分頁與對話框內容均使用繁體中文；不可將英文技術字串直接呈現在使用者操作介面。
2. `lang` 統一使用 `zh-Hant` 或 `zh-TW`；必要的產品名、服務名、代碼（例如 Google、LINE、API、UUID、RBAC）可保留原文，但須搭配繁體中文說明。
3. 搜尋提示、日期／時間提示、無資料與載入訊息也必須是繁體中文。資料庫列舉值、第三方回應與錯誤碼在畫面呈現前要先經中文標籤轉換。
4. 登入與個人資料欄位使用「電子郵件」等中文欄位名稱；英文只保留在實際格式或帳號值中。
5. 新增系統或子系統前，先盤點 `web/lib/modules.ts` 的系統與模組定義及本節語系規格；若要保留英文縮寫，須同步提供中文名稱與用途說明。

本次盤點確認 `web/lib/modules.ts` 為 8 個系統、48 個子系統；日期欄位的繁中顯示由 `web/components/LocalizedDateInput.tsx` 統一處理，避免瀏覽器直接顯示 `yyyy/月/dd`。
...
| SYS-03 駐衛警巡邏 | 巡邏打卡、巡邏點清單、巡檢排班、逾時推播、設備巡檢、立體巡檢雲臺（6） |
| SYS-04 電子交接簿 | 交接紀錄、未結事項、設備概況（3） |
| SYS-05 設備建置 | 設備主檔、保養排程、維修履歷、維護合約、設備文件、年度成本、中央監控、材料主檔（8） |
| SYS-06 專案關係與設備圖臺 | 區域位置表、整合標記系統、平面樓層圖、3D模型圖、3D建模系統、專案關係圖（6） |
...
## 部署規格：推送後自動更新（2026-08-19）

正式網站由 GitHub Pages 的 `.github/workflows/hardened-pages.yml` 管理：

1. 每次推送至 `main` 立即觸發 Pages 建置流程；不依賴手動點擊部署。
2. 建置會執行 V2 靜態輸出、CSP／安全封裝、來源簽章驗證與全站頁面掃描。
3. 建置成功後立即部署至 GitHub Pages，並在部署完成後再掃描線上網址確認版本。
4. 同一時間只保留最新部署（`cancel-in-progress: true`）；連續快速推送時，舊的建置會取消，避免舊版本覆蓋新版本。

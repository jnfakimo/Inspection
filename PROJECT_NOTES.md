...
- 機電交接簿的「工作分類」與「常用工作項目」下拉選單加入真正空白首項，允許不選預設項目並改填補充說明。
- 新增工作選項管理，可新增、修改、停用及恢復分類與項目；刪除採軟停用，避免破壞既有交接紀錄。
- 維修人員清單統一過濾已離職、去識別化及非有效帳號，避免「已離職人員-dac7」等帳號再次顯示。
- 正式資料庫已套用 `20260911193000_mechanical_work_options.sql`，Edge Function、GitHub Pages 與安全檢查皆部署成功。
- 完整 `npm test`、Next.js webpack 96 頁建置及 Deno 型別檢查通過；線上正式靜態資源已讀回並確認包含本次功能。

### 下一步

- 現場可由機電交接簿的「管理工作選項」維護清單；若新增跨系統的可留白下拉欄位，沿用 `BlankSelectOption`，人員清單沿用 `selectableActiveUsers()`。
- 地端 Supabase／IIS 同步屬另一條部署流程；本次只完成雲端正式站與 GitHub Pages 驗證，不把地端主機狀態推定為已同步。

### 踩坑

- 僅以 `status='active'` 篩選人員不足以排除已去識別化的離職帳號，必須同時檢查停用、離職與去識別化特徵。
- 收工時遠端 `main` 已有其他功能提交，先以 fast-forward 取得最新版本後再補紀錄，未回退或覆蓋他人變更。
...
### 2026-09-12 駐衛警巡檢入口補齊 6 個子系統

- 補齊 SYS-03 駐衛警巡檢系統入口頁（`/v2/systems/guardpatrol/`）漏列的「巡邏點清單（points）」與「設備巡檢（records）」圖卡，完成 MODULE 01～06 完整呈現。
- 入口網格調整為桌面 3 欄 × 2 列置中（`repeat(3, 269px)`），平板 2 欄、手機 1 欄滿寬。
- 通過 `system-page-heading-check` 與共用元件測試。

...
import { hasModuleAccess, type ModuleDefinition, type SystemDefinition } from '@/lib/modules';
...
function VehicleHub({ system, profile }: { system: SystemDefinition; profile: Profile }) {
  ...
  const vehicleCards = [
    ['requests', '派車申請', '提出用車申請並追蹤多階段核可流程。', 'vehicle-request-icon-ai.png', 'MODULE 01', '進入系統　→', hasModuleAccess(profile, 'vehicle', 'requests')],
    ['vehicles', '公務車輛', '管理車號、狀態與目前里程資料。', 'vehicle-car-icon-ai.png', 'MODULE 02', '管理車輛　→', canManageFleet && hasModuleAccess(profile, 'vehicle', 'vehicles')],
    ['drivers', '駕駛人員', '維護可指派駕駛與啟用狀態。', 'vehicle-driver-icon-ai.png', 'MODULE 03', '管理駕駛　→', isAdmin && hasModuleAccess(profile, 'vehicle', 'drivers')],
    ['managers', '派車管理員', '設定派車管理與授權人員。', 'vehicle-manager-icon-ai.png', 'MODULE 04', '管理權限　→', isAdmin && hasModuleAccess(profile, 'vehicle', 'managers')],
    ['logs', '派車紀錄', '查詢狀態變更與行車歷程。', 'vehicle-log-icon-ai.png', 'MODULE 05', '查看紀錄　→', hasModuleAccess(profile, 'vehicle', 'logs')],
  ] as const;
...
function OperationsHub({ system, profile }: { system: SystemDefinition; profile: Profile }) {
  ...
  const canSeeModule = (key: string) => hasModuleAccess(profile, system.key, key)
    || (system.key === 'handover' && key === 'guard' && hasModuleAccess(profile, 'handover', 'guard-approve'));
  ...
  if (system.key === 'equipment' || system.key === 'structuremap') {
    return <AppShell profile={profile} title={system.title}
      ...
      <section className={`operations-portal-grid ${system.key}-portal`}>{system.modules.filter(module => canSeeModule(module.key)).map((module, index) => <Link key={module.key} href={`/systems/${system.key}/${module.key}/`} className="operations-portal-card"><div className="operations-portal-card-top"><span className="operations-portal-code">{`MODULE ${String(index + 1).padStart(2, '0')}`}</span><span className="operations-portal-status">● 系統連線</span></div><span className={`generated-module-icon generated-module-icon-${index}`} aria-hidden="true" /><h2>{module.title}</h2><p>{module.description}</p><b>進入系統　→</b></Link>)}</section>
    </AppShell>;
  }

...
function OperationsHub({ system, profile }: { system: SystemDefinition; profile: Profile }) {
  ...
  const visibleHandoverModules = profile.allowed_handover_modules || [];
  ...
  return <AppShell profile={profile} title={system.title}
    ...
    <section className={`operations-portal-grid ${handover ? 'handover' : 'patrol'}`}>{cards.filter(([key]) => !handover || visibleHandoverModules.includes('*') || visibleHandoverModules.includes(key)).map(([key, title, description, icon, code]) => <Link key={key} href={`/systems/${system.key}/${key}/`} className="operations-portal-card"><div className="operations-portal-card-top"><span className="operations-portal-code">{code}</span><span className="operations-portal-status">● 系統連線</span></div><img src={`/Inspection/assets/system-icons-v20260901/${icon}`} alt="" /><h2>{title}</h2><p>{description}</p><b>{handover ? '進入系統　→' : key === 'checkins' ? '進入系統　→' : key === 'map3d' ? '開啟立體檢視　→' : key === 'shifts' ? '管理巡檢班別　→' : '查看通知　→'}</b></Link>)}</section>
    {handover && !visibleHandoverModules.includes('*') && !cards.some(([key]) => visibleHandoverModules.includes(key)) && <div className="notice danger">目前帳號尚未指派任何電子交接簿子系統，請洽系統管理員設定。</div>}
  </AppShell>;
}

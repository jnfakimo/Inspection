...
function repairStatus(value: unknown): string { return ({ pending: '待處理', assigned: '已指派', in_progress: '處理中', pending_review: '待驗收', closed: '已結案', cancelled: '已取消' } as Record<string, string>)[String(value)] || text(value); }
...
function WorkorderHub({ profile }: { profile: Profile }) {
  ...
  return <AppShell profile={profile} title="維修／派工／完工系統">
    ...
    <section className="panel maintenance-overview">
      <div className="maintenance-filters"><select value={repairFilter} onChange={event => setRepairFilter(event.target.value)}><option value="">全部報修狀態</option><option value="pending">待處理</option><option value="assigned">已指派</option><option value="in_progress">處理中</option><option value="closed">已結案</option></select><select value={maintenanceFilter} onChange={event => setMaintenanceFilter(event.target.value)}><option value="">全部維修狀態</option><option value="__none__">未建立工單</option><option value="in_progress">處理中</option><option value="pending_review">待驗收</option><option value="closed">結案</option></select><button className="secondary-btn" onClick={() => void load()}>重新載入</button></div>
      {error && <p className="inline-message danger">{error}</p>}
      <div className="responsive-table"><table><thead><tr><th>報修單編號</th><th>設備</th><th>報修人</th><th>故障說明</th><th>發生位置</th><th>來源</th><th>報修狀態</th><th>維修狀態</th><th>建立時間</th></tr></thead><tbody>{rows.length ? rows.map((row, index) => <tr key={String(row.request_id || row.req_no || index)}><td className="mono cyan-text">{text(row.req_no)}</td><td>{text(row.equipment_id)}</td><td>{text(row.reporter)}</td><td>{text(row.fault_desc).slice(0, 48)}</td><td>{text(row.fault_location)}</td><td>{row.source === 'inspection' ? '巡檢' : '直接通報'}</td><td><span className="status-pill">{repairStatus(row.status)}</span></td><td>{row.maintenance_status ? <span className="status-pill">{repairStatus(row.maintenance_status)}</span> : '—'}</td><td className="mono">{date(row.created_at)}</td></tr>) : <tr><td colSpan={9} className="empty">{error ? '資料載入失敗' : '目前無紀錄'}</td></tr>}</tbody></table></div>
    </section>
  </AppShell>;
}

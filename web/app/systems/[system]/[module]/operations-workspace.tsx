...
function GuardPatrolWorkspace({ system, module, profile }: { system: SystemDefinition; module: ModuleDefinition; profile: Profile }) {
  const client = getSupabase();
  const [rows, setRows] = useState<Row[]>([]); const [points, setPoints] = useState<Point[]>([]); const [selected, setSelected] = useState(''); const [date, setDate] = useState(today()); const [floor, setFloor] = useState(''); const [shift, setShift] = useState(''); const [status, setStatus] = useState(''); const [busy, setBusy] = useState(false); const [note, setNote] = useState('');
  const load = useCallback(async () => { setBusy(true); try { const [data, markerResult] = await Promise.all([invokeAppApi<{ rows: Row[] }>('module_data', { system: 'guardpatrol', module: 'checkins' }), client.from('plan_markers').select('marker_id,floor_id,label,note').eq('kind', 'patrol').order('floor_id').order('label').limit(1000)]); setRows(data.rows || []); setPoints(markerResult.data || []); } catch (error) { setNote(error instanceof Error ? error.message : '巡檢資料載入失敗'); } finally { setBusy(false); } }, [client]);
  useEffect(() => { void load(); }, [load]);
  ...
  return <AppShell profile={profile} title={system.title}><div className='content v1-content operations-page'><header className='operations-header'><img src={system.icon} alt='' /><div><h1>{system.title}</h1><p>依班別排程管理巡邏點，記錄各樓層巡檢打卡與逾期狀態。</p></div></header>{note && <p className='operations-note' role='status'>{note}</p>}<section className='operations-panel'><div className='operations-panel-title'><h2>{module.title}</h2><button onClick={() => void load()} disabled={busy}>重新載入</button></div><p className='operations-hint'>依「班別排程管理」設定的當天班別，對照巡邏點 QR 簽到記錄，即時查看每個巡檢點的稽核狀態。</p><div className='operations-datebar'><button onClick={() => setDate(current => { const value = new Date(current + 'T00:00:00'); value.setDate(value.getDate() - 1); return value.toISOString().slice(0, 10); })}>◀</button><input type='date' value={date} onChange={e => setDate(e.target.value)} /><button onClick={() => setDate(current => { const value = new Date(current + 'T00:00:00'); value.setDate(value.getDate() + 1); return value.toISOString().slice(0, 10); })}>▶</button><button onClick={() => setDate(today())}>今天</button></div><div className='operations-filter-row'><label>樓層<select value={floor} onChange={e => setFloor(e.target.value)}><option value=''>全部樓層</option>{floors.map(value => <option key={value} value={value}>{value}</option>)}</select></label><label>班別<select value={shift} onChange={e => setShift(e.target.value)}><option value=''>全部班別</option>{patrolShifts.map(item => <option key={item.id} value={item.id}>{item.label} {item.time}</option>)}</select></label><label>打卡狀態<select value={status} onChange={e => setStatus(e.target.value)}><option value=''>全部狀態</option><option value='ok'>已打卡</option><option value='pending'>待打卡</option><option value='overdue'>逾期未打卡</option></select></label><button className='operations-export' onClick={() => exportCsv(visibleRows)}>匯出當日 CSV</button><button className='operations-export' onClick={() => exportCsv(rows)}>匯出期間 CSV</button></div><div className='operations-stat-row'><span>{shift ? patrolShifts.find(item => item.id === shift)?.label : '當班'}<b>{shift ? patrolShifts.find(item => item.id === shift)?.time : '全部班別'}</b></span><span className='stat-done'>已打卡 <b>{visibleRows.length}</b></span><span className='stat-pending'>待打卡 <b>{Math.max(points.length - visibleRows.length, 0)}</b></span><span className='stat-overdue'>逾期未打卡 <b>0</b></span></div><div className='operations-legend'><span className='ok'>● 已打卡</span><span className='pending'>● 待打卡（班別進行中）</span><span className='overdue'>● 逾期未打卡</span></div><div className='operations-table-wrap'><table className='operations-matrix'><thead><tr><th>樓層</th><th>巡檢點</th>{patrolShifts.map(item => <th key={item.id}>{item.label}<br />{item.time}</th>)}</tr></thead><tbody>{floorGroups.map(group => <tr key={group.floor}><td className='floor-name'>▶ {group.floor}</td><td>{group.points.length} 個巡檢點</td>{patrolShifts.map(item => <td key={item.id}>{group.points.some(point => visibleRows.some(row => String(row.floor_id || '未分類') === group.floor && String(row.label || '') === String(point.label || ''))) ? <span className='operations-dot done'>●</span> : <span className='operations-dot pending'>●</span>}</td>)}</tr>)}</tbody></table></div><div className='operations-checkin-bar'><label>巡檢點<select value={selected} onChange={e => setSelected(e.target.value)}><option value=''>— 選擇巡檢點 —</option>{points.map(point => <option key={point.marker_id} value={point.marker_id}>{point.floor_id || '未分類'}｜{point.label || point.marker_id}</option>)}</select></label><button className='primary' disabled={busy} onClick={() => void checkin()}>✓ 完成打卡</button></div><div className='operations-table-wrap'><table className='operations-table'><thead><tr><th>打卡時間</th><th>巡檢人員</th><th>樓層</th><th>巡檢點</th><th>類型</th><th>狀態</th></tr></thead><tbody>{visibleRows.map((row, index) => <tr key={String(row.checkin_id || index)}><td>{text(row.checkin_at)}</td><td>{text(row.user_name)}</td><td>{text(row.floor_id)}</td><td>{text(row.label)}</td><td>{text(row.target_type)}</td><td><span className='operations-status done'>已打卡</span></td></tr>)}</tbody></table>{!visibleRows.length && <p className='operations-empty'>{busy ? '載入中…' : '目前沒有符合條件的巡檢資料。'}</p>}</div></section></div></AppShell>;
}

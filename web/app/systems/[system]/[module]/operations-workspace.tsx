...
// SYS-03 的 points／records／shifts／notifications 已改由 PatrolWorkspace 承接
// （原本 points 與 records 都誤渲染成打卡畫面，shifts 繞過 save_patrol_shift，
// notifications 走 module_data 導致多數欄位取不到）。checkins 維持既有打卡矩陣，
// map3d 仍暫用 GuardPatrolSpecialized 的清單版，待 SYS-06 圖臺原生化後一併處理。
const PATROL_MODULES = new Set(['points', 'records', 'shifts', 'notifications']);
...
export function OperationsWorkspace({ system, module }: { system: SystemDefinition; module: ModuleDefinition }) {
  return <AuthGate>{profile => system.key === 'handover'
    ? <HandoverWorkspace system={system} module={module} profile={profile} />
    : PATROL_MODULES.has(module.key)
      ? <PatrolWorkspace system={system} module={module} profile={profile} />
      : module.key === 'map3d'
        ? <GuardPatrolSpecialized system={system} module={module} profile={profile} />

...
function GuardPatrolWorkspace({ system, module, profile }: { system: SystemDefinition; module: ModuleDefinition; profile: Profile }) {
  const client = getSupabase();
  const [rows, setRows] = useState<Row[]>([]); const [points, setPoints] = useState<Point[]>([]); const [selected, setSelected] = useState(''); const [date, setDate] = useState(today()); const [floor, setFloor] = useState(''); const [status, setStatus] = useState(''); const [busy, setBusy] = useState(false); const [note, setNote] = useState('');
  ...
  useEffect(() => { void load(); }, [load]);
  const visibleRows = useMemo(() => rows.filter(row => (!floor || String(row.floor_id || '') === floor) && (!date || String(row.checkin_at || '').slice(0, 10) === date) && (!status || status === 'ok')), [rows, floor, date, status]);
  ...
  return <AppShell profile={profile} title={system.title}><div className='content v1-content operations-page'><header className='operations-header'><img src={system.icon} alt='' /><div><h1>{system.title}</h1><p>依 V1 巡檢表流程，依樓層與巡檢點記錄當班打卡狀態。</p></div></header>{note && <p className='operations-note' role='status'>{note}</p>}<section className='operations-panel'><div className='operations-panel-title'><h2>{module.title}</h2><button onClick={() => void load()} disabled={busy}>重新載入</button></div><p className='operations-hint'>依班別排程對照巡邏點 QR 簽到記錄，即時查看每個巡檢點的稽核狀態。</p><div className='operations-datebar'><button onClick={() => setDate(current => { const value = new Date(current + 'T00:00:00'); value.setDate(value.getDate() - 1); return value.toISOString().slice(0, 10); })}>◀</button><input type='date' value={date} onChange={e => setDate(e.target.value)} /><button onClick={() => setDate(current => { const value = new Date(current + 'T00:00:00'); value.setDate(value.getDate() + 1); return value.toISOString().slice(0, 10); })}>▶</button><button onClick={() => setDate(today())}>今天</button></div><div className='operations-filter-row'><label>樓層<select value={floor} onChange={e => setFloor(e.target.value)}><option value=''>全部樓層</option>{floors.map(value => <option key={value} value={value}>{value}</option>)}</select></label><label>打卡狀態<select value={status} onChange={e => setStatus(e.target.value)}><option value=''>全部狀態</option><option value='ok'>已打卡</option><option value='pending'>待打卡</option><option value='overdue'>逾期未打卡</option></select></label></div><div className='operations-stat-row'><span>當日已打卡 <b>{visibleRows.length}</b></span><span>巡檢點總數 <b>{points.length}</b></span><span>待打卡 <b>{Math.max(points.length - visibleRows.length, 0)}</b></span></div><div className='operations-legend'><span className='ok'>● 已打卡</span><span className='pending'>● 待打卡</span><span className='overdue'>● 逾期未打卡</span></div><div className='operations-table-wrap'><table className='operations-table'><thead><tr><th>打卡時間</th><th>巡檢人員</th><th>樓層</th><th>巡檢點</th><th>類型</th><th>狀態</th></tr></thead><tbody>{visibleRows.map((row, index) => <tr key={String(row.checkin_id || index)}><td>{text(row.checkin_at)}</td><td>{text(row.user_name)}</td><td>{text(row.floor_id)}</td><td>{text(row.label)}</td><td>{text(row.target_type)}</td><td><span className='operations-status done'>已打卡</span></td></tr>)}</tbody></table>{!visibleRows.length && <p className='operations-empty'>{busy ? '載入中…' : '目前沒有符合條件的巡檢資料。'}</p>}</div></section></div></AppShell>;
}

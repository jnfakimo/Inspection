...
function HandoverWorkspace({ system, module, profile }: { system: SystemDefinition; module: ModuleDefinition; profile: Profile }) {
  ...
  return <AppShell><div className='content v1-content operations-page'>
...
function GuardPatrolWorkspace({ system, module, profile }: { system: SystemDefinition; module: ModuleDefinition; profile: Profile }) {
  const client = getSupabase();
  const [rows, setRows] = useState<Row[]>([]); const [points, setPoints] = useState<Point[]>([]); const [selected, setSelected] = useState(''); const [busy, setBusy] = useState(false); const [note, setNote] = useState('');
  ...
  return <AppShell><div className='content v1-content operations-page'><header className='operations-header'><img src={system.icon} alt='' /><div><h1>{system.title}</h1><p>依 V1 巡檢表流程，依樓層與巡檢點記錄當班打卡狀態。</p></div></header>{note && <p className='operations-note' role='status'>{note}</p>}<section className='operations-panel'><div className='operations-panel-title'><h2>{module.title}</h2><button onClick={() => void load()} disabled={busy}>重新載入</button></div><div className='operations-checkin-bar'><label>巡檢點<select value={selected} onChange={e => setSelected(e.target.value)}><option value=''>— 選擇巡檢點 —</option>{points.map(point => <option key={point.marker_id} value={point.marker_id}>{point.floor_id || '未分類'}｜{point.label || point.marker_id}</option>)}</select></label><button className='primary' disabled={busy} onClick={() => void checkin()}>✓ 完成打卡</button></div><div className='operations-table-wrap'><table className='operations-table'><thead><tr><th>打卡時間</th><th>巡檢人員</th><th>樓層</th><th>巡檢點</th><th>類型</th></tr></thead><tbody>{rows.map((row, index) => <tr key={String(row.checkin_id || index)}><td>{text(row.checkin_at)}</td><td>{text(row.user_name)}</td><td>{text(row.floor_id)}</td><td>{text(row.label)}</td><td>{text(row.target_type)}</td></tr>)}</tbody></table>{!rows.length && <p className='operations-empty'>{busy ? '載入中…' : '目前沒有巡檢打卡紀錄。'}</p>}</div></section></div></AppShell>;
}

...
import { checkinRange, getPatrolShiftsForDate, isDeletedShift, notificationRange, type PatrolShift } from '@/lib/patrol-status';
...
function GuardPatrolWorkspace({ system, module, profile }: { system: SystemDefinition; module: ModuleDefinition; profile: Profile }) {
  ...
  const statusFor = (point: Point, schedule: PatrolSchedule) => {
    ...
    const checked = rows.some(row => {
      if (String(row.checkin_at || '').slice(0, 10) !== date) return false;
      ...
      // 舊資料沒有 shift_type 時，以排班的「班別／通報時段聯集」回推歸屬，
      // 避免同一天其他班別的打卡把本班錯算成已完成。
      if (!row.shift_type) {
        const checkinAt = new Date(String(row.checkin_at || ''));
        if (Number.isNaN(checkinAt.getTime()) || checkinAt < accepted.start || checkinAt > accepted.end) return false;
      }
      return true;
    });
    ...
    return day < todayDay || (day.getTime() === todayDay.getTime() && now >= notice.end) ? 'overdue' : 'pending';
  };

  ...
  function Workspace({ profile }: { profile: Profile }) {
    ...
    return <AppShell profile={profile} title={module.title}>
      ...
      <section className={`panel table-panel ${system.key === 'workorder' && module.key === 'requests' ? 'request-v1-table' : ''}`}>
        ...
        {!data && !error ? <div className="loading-panel">正在透過安全服務載入資料…</div> : <div className="responsive-table"><table><thead><tr>{data?.columns.map(column => <th key={column.key}>{zhValue(column.label)}</th>)}{system.key === 'workorder' && module.key === 'dispatch' && <th>操作</th>}{system.key === 'workorder' && module.key === 'requests' && <th>檢視</th>}</tr></thead><tbody>{rows.map((row, index) => { const action = nextRepairAction(String(row.status || 'pending')); return <tr key={String(row.id || row.request_id || row.record_id || row.user_id || index)} onClick={() => system.key === 'workorder' && module.key === 'requests' && setSelectedRow(row)}>{data?.columns.map(column => <td key={column.key}>{display(row[column.key])}</td>)}{system.key === 'workorder' && module.key === 'dispatch' && <td><button className="secondary-btn" onClick={event => { event.stopPropagation(); if (action) void updateRepairStatus(row, action[0]); }}>{action ? action[1] : '—'}</button></td>}{system.key === 'workorder' && module.key === 'requests' && <td className="request-view-link">檢視 ›</td>}</tr>; })}</tbody></table>{data && rows.length === 0 && <p className="empty">查無資料</p>}</div>}
      </section>
      {selectedRow && <div className="request-detail-backdrop" role="dialog" aria-modal="true"><section className="request-detail-modal"><header><h2>案件詳情</h2><button onClick={() => setSelectedRow(null)} aria-label="關閉">×</button></header><div className="request-detail-grid"><div><span>報修單號</span><strong>{display(selectedRow.req_no)}</strong></div><div><span>報修人</span><strong>{display(selectedRow.reporter)}</strong></div><div><span>所屬單位</span><strong>{display(selectedRow.department)}</strong></div><div><span>聯絡手機</span><strong>{display(selectedRow.mobile)}</strong></div><div><span>故障類型</span><strong>{display(selectedRow.fault_type)}</strong></div><div><span>急迫度</span><strong>{display(selectedRow.urgency)}</strong></div><div className="full"><span>故障位置</span><strong>{display(selectedRow.fault_location)}</strong></div><div className="full"><span>故障說明</span><p>{display(selectedRow.fault_desc)}</p></div><div><span>報修狀態</span><strong>{display(selectedRow.status)}</strong></div><div><span>建立時間</span><strong>{display(selectedRow.created_at)}</strong></div></div></section></div>}

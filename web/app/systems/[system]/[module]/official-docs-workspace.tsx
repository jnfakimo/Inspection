...
function OfficialDocsPage({ profile, system, module }: { profile: Profile; system: SystemDefinition; module: ModuleDefinition }) {
  ...
  const timelinePageCount = Math.max(1, Math.ceil((selected?.events.length || 0) / TIMELINE_PAGE_SIZE));
  const pagedTimelineEvents = useMemo(() => (selected?.events || []).slice((timelinePage - 1) * TIMELINE_PAGE_SIZE, timelinePage * TIMELINE_PAGE_SIZE), [selected?.events, timelinePage]);
  useEffect(() => { setTimelinePage(1); }, [selected?.document_id]);
  useEffect(() => { if (timelinePage > timelinePageCount) setTimelinePage(timelinePageCount); }, [timelinePage, timelinePageCount]);
  useEffect(() => {

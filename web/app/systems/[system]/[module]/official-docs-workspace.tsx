...
type Person = { user_id: string; name: string; dept_id?: string | null; role?: string | null; rbac_role?: string | null; department?: string | null };
...
type Document = { document_id: string; document_no: string; subject: string; originator_id: string; originator_dept_id?: string | null; originator_name?: string | null; status: string; current_step_id?: string | null; barcode_value?: string | null; created_at: string; updated_at: string; closed_at?: string | null; steps: Step[]; events: Event[] };
type OfficialData = { documents: Document[]; departments: Department[]; people: Person[] };
...
function OfficialDocsPage({ profile, system, module }: { profile: Profile; system: SystemDefinition; module: ModuleDefinition }) {
  ...
  const [form, setForm] = useState({ subject: '', document_no: '' });
  ...
  const createDocument = async (event: React.FormEvent) => {
    ...
    if (!form.subject.trim() || !form.document_no.trim()) { setNote('請填寫公文主旨與公文編號'); return; }
    setSaving(true); setNote('');
    try {
      const result = await invokeAppApi<{ document_id: string }>('official_document_create', form);
      setForm({ subject: '', document_no: '' });
      setNote('公文已建立，請選擇下一個會辦或陳核部室送出');
      await load('');
  ...
  return <AppShell profile={profile} title={system.title}>
    <div className="od-page">
      <header className="od-header"><div><span className="eyebrow">{system.code} · {module.title}</span><h1>{system.title}</h1><p>以部室為單位傳送公文，依序完成會辦、陳核、核決與原申請人收訖；所有動作保留不可刪除的時間軸。</p></div><div className="od-header-badge"><strong>{data?.documents.length ?? 0}</strong><span>可查詢公文</span></div></header>
      ...
      <section className="panel od-create-panel"><div className="od-section-title"><div><span className="od-step-no">03</span><h2>建立公文</h2></div><small>附件與電子簽章內容列入後續版本；本版先建立可追蹤流程</small></div><form className="od-create-form" onSubmit={createDocument}><label>公文編號<input value={form.document_no} onChange={event => setForm(current => ({ ...current, document_no: event.target.value }))} maxLength={100} required placeholder="例如：北農字第 1150001 號" /></label><label>公文主旨<input value={form.subject} onChange={event => setForm(current => ({ ...current, subject: event.target.value }))} maxLength={300} required placeholder="請輸入公文主旨" /></label><button type="submit" className="primary-btn" disabled={saving}>建立並產生查詢條碼</button></form><p className="od-help">建立後由單位公文管理人員選擇下一個會辦部室或陳核部室；每個動作都會留下操作者、角色、部室與時間。</p></section>
    </div>

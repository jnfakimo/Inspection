...
function OfficialDocsPage({ profile, system, module }: { profile: Profile; system: SystemDefinition; module: ModuleDefinition }) {
  ...
  return <AppShell profile={profile} title={system.title}>
    <div className="od-page">
      ...
      {canViewPeople && <section className="panel od-people-panel"><div className="od-section-title"><div><span className="od-step-no">04</span><h2>單位人員資料</h2></div><small>僅顯示本人單位及下屬單位；一般使用者不顯示此區</small></div>{data?.people.length ? <div className="od-people-table responsive-table"><table><thead><tr><th>姓名</th><th>單位</th><th>角色</th></tr></thead><tbody>{data.people.map(person => <tr key={person.user_id}><td>{person.name}</td><td>{person.department || '—'}</td><td>{roleLabel(person.rbac_role || person.role)}</td></tr>)}</tbody></table></div> : <p className="empty">目前單位沒有可顯示的人員資料。</p>}</section>}
    </div>

...
const STATUS_LABELS: Record<string, string> = {
  draft: '待送出', awaiting_co_sign: '待會辦收文', ready_for_next: '會辦完成／待選下一站', awaiting_approval: '待陳核簽收／核決', awaiting_originator: '待原申請人收訖', returned: '已退回待補正', closed: '已結案',
};
...
const isReceivedByRoutingUnit = (document: Document) => {
  const currentStep = document.steps.find(step => step.step_id === document.current_step_id);
  return Boolean(currentStep && ['co_sign', 'approval'].includes(currentStep.step_type) && ['received', 'completed'].includes(currentStep.status));
};
  ...
  useEffect(() => {
    ...
    const start = async () => {
      ...
      try {
        ...
        if (active) setMessage('鏡頭已開啟，請將公文一維條碼置於掃描框內。');
        else controls.stop();
      } catch (error) {
        if (!active) return;
...
function QrModal({ document, onClose }: { document: Document; onClose: () => void }) {
  ...
  const download = () => {
    ...
    source.onload = () => {
      ...
      link.download = `${document.document_no}-公文條碼.png`;
      link.click();
    };
    source.src = image;
  };
...
function OfficialDocsPage({ profile, system, module }: { profile: Profile; system: SystemDefinition; module: ModuleDefinition }) {
  ...
  const [form, setForm] = useState({ document_type: 'official_document', subject: '', root_department_id: '', responsible_dept_id: '', responsible_user_id: '' });
  ...
  const departmentOptions = useMemo(() => (data?.departments || []).filter(department => !department.parent_id).map(department => ({ value: department.dept_id, label: department.code ? `${department.name}（${department.code}）` : department.name })), [data?.departments]);
  ...
  const visibleDocuments = useMemo(() => (data?.documents || []).filter(document => !isReceivedByRoutingUnit(document)), [data?.documents]);
  const hiddenDocumentCount = Math.max(0, (data?.documents.length || 0) - visibleDocuments.length);
  ...
  const createDocument = async (event: React.FormEvent) => {
    ...
    if (!form.responsible_dept_id || !form.responsible_user_id) { setNote('請選擇第二階單位與承辦人員'); return; }
    setSaving(true); setNote('');
    try {
      const result = await invokeAppApi<{ document_id: string }>('official_document_create', form);
      setForm(current => ({ ...current, subject: '', responsible_dept_id: '', responsible_user_id: '' }));

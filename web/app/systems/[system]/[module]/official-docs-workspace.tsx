...
function OfficialDocsPage({ profile, system, module }: { profile: Profile; system: SystemDefinition; module: ModuleDefinition }) {
  ...
  const departmentOptions = useMemo(() => (data?.departments || []).filter(department => !department.parent_id).map(department => ({ value: department.dept_id, label: department.code ? `${department.name}（${department.code}）` : department.name, ...unitCapabilities(department) })), [data?.departments]);
  ...
  const openBarcode = async () => {
    ...
    try {
      ...
      setNote('條碼已產生，可直接列印或下載');
      await load(lookup);
    } catch (error) { setNote(error instanceof Error ? error.message : '條碼產生失敗'); }
    finally { setSaving(false); }
  };

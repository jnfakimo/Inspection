...
export function MechanicalHandover({ system, module, profile }: Props) {
  ...
  return <AppShell profile={profile} title={module.title} heading={{ system, module, title: module.title, metaTitle: system.title }}>
        ...
        {SHIFTS.map((shift, index) => {
          ...
          return <section className={`mechanical-shift mechanical-shift-${index + 1}${isCurrent ? ' is-current' : ''}`} key={shift.code}>
            <div className="mechanical-shift-head">
              <div className="mechanical-shift-title"><strong>{['一', '二', '三'][index]}</strong><span><b>{['早班', '中班', '晚班'][index]}{isCurrent && <em>目前當班</em>}</b><small>{shift.label} · {rows.length} 件 · 費用 {formatRepairCost(repairCostTotal(rows))}</small></span></div>
              <button className="primary-btn compact" disabled={busy || Boolean(approval)} onClick={() => { setCarrySource(null); setEditingShift(shift.code); }}>＋ 新增工作</button>
            </div>
            {carryRows.length > 0 && <div className="mechanical-carry-list" aria-label={`${shift.label}上班續辦工作`}>
              ...
              {carryRows.map(row => <article className="mechanical-carry-card" key={String(row.entry_id)}>
                ...
                <button className="secondary-btn compact" disabled={busy || Boolean(approval)} onClick={() => { setCarrySource(row); setEditingShift(shift.code); }}>接續處理</button>
              </article>)}
            </div>}
    ...
    {editingShift && <WorkEntryModal date={date} shiftCode={editingShift} users={mechanicalUsers} profile={profile} presetItem={presetItem} carrySource={carrySource} onClose={() => { setEditingShift(null); setPresetItem(''); setCarrySource(null); }} onDone={async () => { setEditingShift(null); setPresetItem(''); setCarrySource(null); await load(); setNote(carrySource ? '上班未完成工作已建立續辦紀錄' : '維修養護工作已新增'); }} />}
  </AppShell>;
}
...
export function WorkEntryModal({ date, shiftCode, users, profile: _profile, presetItem = '', carrySource, onClose, onDone }: { date: string; shiftCode: string; users: Row[]; profile: Profile; presetItem?: string; carrySource?: Row | null; onClose: () => void; onDone: () => void }) {
  const sourceItem = String(carrySource?.work_item || presetItem || '');
  const initialCategory = String(carrySource?.category || Object.keys(WORK_ITEMS).find(category => WORK_ITEMS[category].includes(sourceItem)) || Object.keys(WORK_ITEMS)[0]);
  const [category, setCategory] = useState(initialCategory);
  const [item, setItem] = useState(sourceItem || WORK_ITEMS[initialCategory]?.[0] || '');
  const [details, setDetails] = useState(String(carrySource?.details || ''));
  const [technicians, setTechnicians] = useState<string[]>(Array.isArray(carrySource?.technician_ids) ? carrySource.technician_ids.map(String) : []);
  const [result, setResult] = useState(carrySource ? '處理中' : '正常');
  const [notes, setNotes] = useState(String(carrySource?.notes || ''));
  ...
  const submit = async () => {
    ...
    try {
      await invokeAppApi('handover_save', { kind: 'mechanical_entry', work_date: date, shift_code: shiftCode, category, work_item: item, details, technician_ids: technicians, result, notes, repair_cost: repairCost.trim() || null, carry_source_id: carrySource?.entry_id || null });
      await onDone();
  ...
  return <AdminModal className="mechanical-modal mechanical-work-modal" title={`${carrySource ? '接續處理' : '新增機電工作'}｜${SHIFTS.find(shift => shift.code === shiftCode)?.label}`} onClose={onClose}>
    ...
    <div className="admin-form-grid mechanical-form">
      ...
      <label>常用工作項目<select value={item} onChange={event => setItem(event.target.value)}>{itemOptions.map(value => <option key={value}>{value}</option>)}</select></label>
    ...
    <footer><button className="secondary-btn" onClick={onClose}>取消</button><button className="primary-btn compact" disabled={busy} onClick={() => void submit()}>{busy ? '儲存中…' : carrySource ? '建立續辦紀錄' : '新增工作紀錄'}</button></footer>
  </AdminModal>;
}

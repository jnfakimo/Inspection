...
function EquipmentMap({ profile }: { profile: Profile }) {
  ...
  return <AppShell profile={profile} title="設備圖臺">
    <div className="page-actions"><div><p>設備主檔、平面圖標記與位置資料統一呈現；地下室樓層代碼由系統服務統一處理。</p>{error && <span className="inline-message danger">{error}</span>}</div><a className="primary-btn compact" href="/Inspection/system/b1plan.html">開啟高精度平面圖臺</a></div>

...
function renderAuditFlow(row: Row) {
  ...
  } else if (c.event_type === 'function_use' && c.feature) {
    summary = `使用者執行了功能：${c.feature}`;
  }
  ...
  return (
    <div className="audit-flow-box" style={{ padding: '15px', background: 'rgba(128,128,128,0.1)', borderRadius: '8px', marginBottom: '15px' }}>
...
export function AuditAdminV2({ profile, module }: AdminProps) {
  ...
  return <AppShell profile={profile} title={module.title}>
    ...
    <section className="panel admin-panel"><div className="admin-toolbar audit-filters"><input value={query} onChange={event => changeFilter(setQuery, event.target.value)} placeholder="搜尋資源、動作、紀錄編號或來源"/><select value={resource} onChange={event => changeFilter(setResource, event.target.value)}><option value="">全部資源</option>{RESOURCES.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><select value={action} onChange={event => changeFilter(setAction, event.target.value)}><option value="">全部操作</option>{ACTIONS.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><label>起日<LocalizedDateInput aria-label="起始日期（年／月／日）" value={from} onChange={event => changeFilter(setFrom, event.target.value)}/></label><label>迄日<LocalizedDateInput aria-label="結束日期（年／月／日）" value={to} onChange={event => changeFilter(setTo, event.target.value)}/></label><button className="secondary-btn" onClick={clear}>清除</button></div>
    ...
    {detail && <AdminModal title="操作稽核詳細內容" onClose={() => setDetail(null)}>
      ...
      <details style={{ marginTop: '1rem', cursor: 'pointer', background: 'rgba(0,0,0,0.2)', padding: '0.5rem', borderRadius: '4px' }}>
        <summary style={{ fontWeight: 'bold', userSelect: 'none' }}>查看完整紀錄代碼 (JSON)</summary>
        <pre style={{ marginTop: '0.5rem', maxHeight: '400px', overflow: 'auto' }}>{JSON.stringify(detail, null, 2)}</pre>
      </details>

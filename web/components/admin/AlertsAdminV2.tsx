...
export function AlertsAdminV2({ profile, module }: AdminProps) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const { data, error } = await getSupabase().from('security_alerts').select('*').order('last_seen_at', { ascending: false }).limit(1000);
      if (error) setNote(`失敗：${errorMessage(error, '資安告警載入失敗')}`);
      setAlerts(data || []);
    } catch (error) { setNote(`失敗：${errorMessage(error, '資安告警載入失敗')}`); }
  ...
  const alertPages = Math.max(1, Math.ceil(rows.length / PAGE_SIZE));
  useEffect(() => { if (page > alertPages) setPage(alertPages); }, [page, alertPages]);

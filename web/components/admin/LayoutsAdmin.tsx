...
export function LayoutsAdmin({ profile, module }: AdminProps) {
  ...
  const load = useCallback(async (options: { preferredVersionId?: string; preserveNote?: boolean } = {}) => {
    setBusy(true); if (!options.preserveNote) setNote(''); const client = getSupabase();
    ...
    setSelected(preferred); setItems((grouped[preferred] || []).map(row => ({ ...row }))); setDirty(false); setBusy(false);
  }, [selected]);

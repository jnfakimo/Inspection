...
import { AdminHeader, AdminModal, type AdminProps, errorMessage, type Row, StatusPill } from './shared';
...
export function LocationsAdmin({ profile, module }: AdminProps) {
  ...
  const [tab, setTab] = useState<'locations' | 'departments'>('locations'), [busy, setBusy] = useState(true), [note, setNote] = useState(''), [query, setQuery] = useState(''), [editor, setEditor] = useState<Row | null>(null);
  const load = useCallback(async () => {
    ...
    setLocations(locationResult.data || []); setDepartments(departmentResult.data || []); setMarkets(marketResult.data || []); setBusy(false);
  }, []);
  ...
  return <AppShell profile={profile} title={module.title}>
    ...
    <section className="panel admin-panel"><div className="admin-tabs"><button className={tab === 'locations' ? 'active' : ''} onClick={() => { setTab('locations'); setEditor(null); }}>場域位置</button><button className={tab === 'departments' ? 'active' : ''} onClick={() => { setTab('departments'); setEditor(null); }}>組織部門</button></div><div className="admin-toolbar"><input value={query} onChange={event => setQuery(event.target.value)} placeholder={`搜尋${tab === 'locations' ? '市場、樓層、區域或細部位置' : '部門名稱或代碼'}`}/><span>共 {tab === 'locations' ? locationRows.length : departmentRows.length} 筆</span></div>
      {tab === 'locations' ? <div className="responsive-table"><table><thead><tr><th>市場</th><th>樓層</th><th>區域</th><th>細部位置</th><th>狀態</th><th>操作</th></tr></thead><tbody>{locationRows.map(row => <tr key={row.location_id}><td>{markets.find(market => market.market_id === row.market_id)?.name || '—'}</td><td>{row.floor}</td><td>{row.area}</td><td>{row.detail || '—'}</td><td><StatusPill value={row.status}/></td><td><div className="admin-row-actions"><button onClick={() => setEditor({ ...row })}>編輯</button><button className={row.status === 'active' ? 'warn' : ''} onClick={() => window.confirm(`確定${row.status === 'active' ? '停用' : '啟用'}此位置？`) && void run('admin_toggle_location', { location_id: row.location_id, status: row.status === 'active' ? 'inactive' : 'active' }, row.status === 'active' ? '位置已停用' : '位置已啟用')}>{row.status === 'active' ? '停用' : '啟用'}</button></div></td></tr>)}</tbody></table></div> : <div className="responsive-table"><table><thead><tr><th>部門名稱</th><th>代碼</th><th>上層部門</th><th>層級</th><th>狀態</th><th>操作</th></tr></thead><tbody>{departmentRows.map(row => <tr key={row.dept_id}><td><strong>{row.name}</strong></td><td>{row.code || '—'}</td><td>{departments.find(dept => dept.dept_id === row.parent_id)?.name || '—'}</td><td>{row.level || 1}</td><td><StatusPill value={row.status}/></td><td><div className="admin-row-actions"><button onClick={() => setEditor({ ...row })}>編輯</button><button className={row.status === 'active' ? 'warn' : ''} onClick={() => window.confirm(`確定${row.status === 'active' ? '停用' : '啟用'}「${row.name}」？`) && void run('admin_toggle_department', { dept_id: row.dept_id, status: row.status === 'active' ? 'inactive' : 'active' }, row.status === 'active' ? '部門已停用' : '部門已啟用')}>{row.status === 'active' ? '停用' : '啟用'}</button></div></td></tr>)}</tbody></table></div>}
    </section>

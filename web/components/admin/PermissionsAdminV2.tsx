...
export function PermissionsAdminV2({ profile, module }: AdminProps) {
  ...
  const matrix = (items: ReadonlyArray<readonly [string, string]>) => <div className="responsive-table permission-matrix"><table><thead><tr><th>角色</th>{items.map(item => <th key={item[0]}>{item[1]}</th>)}</tr></thead><tbody>{roles.map(role => <tr key={role.role_id}><td><strong>{role.name}</strong><div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginTop: '4px' }}><small style={{ margin: 0, opacity: 0.7 }}>{role.role_id}</small><button type="button" onClick={() => setEditor({ ...role, is_edit: true })} style={{ padding: '2px 8px', fontSize: '11px', background: 'transparent', border: '1px solid var(--cyan)', color: 'var(--cyan)', borderRadius: '4px', cursor: 'pointer', lineHeight: 1 }}>編輯</button></div></td>{items.map(item => {

...
import { AdminHeader, AdminModal, type AdminProps, errorMessage, HANDOVER_MODULE_PERMISSIONS, PERMISSIONS, roleLabel, type Row, StatusPill, SYSTEM_PERMISSIONS, userRole } from './shared';
...
export function PermissionsAdminV2({ profile, module }: AdminProps) {
  ...
  const [tab, setTab] = useState<'matrix' | 'systems' | 'handover' | 'users'>('matrix'), [busy, setBusy] = useState(true), [note, setNote] = useState(''), [query, setQuery] = useState('');
  ...
  return <AppShell profile={profile} title={module.title}>
    ...
    <section className="panel admin-panel"><div className="admin-tabs"><button className={tab === 'matrix' ? 'active' : ''} onClick={() => setTab('matrix')}>功能權限矩陣</button><button className={tab === 'systems' ? 'active' : ''} onClick={() => setTab('systems')}>系統存取權限</button><button className={tab === 'handover' ? 'active' : ''} onClick={() => setTab('handover')}>交接簿子系統</button><button className={tab === 'users' ? 'active' : ''} onClick={() => setTab('users')}>使用者角色指派</button></div>{tab === 'matrix' && matrix(PERMISSIONS)}{tab === 'systems' && matrix(SYSTEM_PERMISSIONS)}
      {tab === 'handover' && <><div className="admin-toolbar"><input value={query} onChange={event => setQuery(event.target.value)} placeholder="搜尋姓名、帳號、單位或角色"/><span>先由角色開放「電子交接簿」，再逐人勾選子系統。</span></div><div className="responsive-table handover-access-table"><table><thead><tr><th>使用者</th><th>單位／角色</th>{HANDOVER_MODULE_PERMISSIONS.map(item => <th key={item[0]}>{item[1]}</th>)}</tr></thead><tbody>{filteredUsers.filter(user => user.status === 'active').map(user => { const roleId = userRole(user), isSystemAdmin = roleId === 'sysadmin', hasParent = isAllowed(roleId, 'sys_handover'); return <tr key={user.user_id}><td><strong>{user.name}</strong><small>{user.username || user.email || '—'}</small></td><td>{user.department || '—'}<small>{roleLabel(roleId, roles)}{hasParent ? '' : '｜未開大系統'}</small></td>{HANDOVER_MODULE_PERMISSIONS.map(([moduleKey, label]) => <td key={moduleKey}><input type="checkbox" aria-label={`${user.name} ${label}`} checked={isSystemAdmin || handoverAllowed(user.user_id, moduleKey)} disabled={busy || !hasParent || isSystemAdmin} onChange={event => void run('admin_set_handover_module_access', { user_id: user.user_id, module_key: moduleKey, allowed: event.target.checked }, '交接簿子系統權限已更新')} />{isSystemAdmin ? <small>管理員全開</small> : !hasParent && <small>先開大系統</small>}</td>)}</tr>; })}</tbody></table></div>{filteredUsers.filter(user => user.status === 'active').length === 0 && <p className="empty">沒有符合條件的啟用帳號。</p>}</>}

...
export function UsersAdmin({ profile, module }: AdminProps) {
  ...
  const run = async (payload: AdminAction, success: string) => {
    ...
    try { await invokeAdminApi(payload.action, payload); setNote(success); setEditor(null); setPasswordUser(null); setPassword(''); setPassword2(''); await load(); }
  ...
  const saveUser = async () => {
    ...
    if (creating && (!String(editor.email || '').trim() || String(editor.password || '').length < 6)) { setNote('失敗：請填寫 Email，初始密碼至少 6 個字元'); return; }
  ...
  return <AppShell profile={profile} title={module.title}>
    ...
    <section className="panel admin-panel"><div className="admin-toolbar"><input value={query} onChange={event => setQuery(event.target.value)} placeholder="搜尋姓名、帳號、Email、單位或角色"/><select value={status} onChange={event => setStatus(event.target.value)}><option value="">全部狀態</option><option value="active">啟用</option><option value="inactive">停用</option></select><span>啟用 {users.filter(user => user.status === 'active').length}／停用 {users.filter(user => user.status === 'inactive').length}</span></div>

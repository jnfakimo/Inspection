...
export function UsersAdmin({ profile, module }: AdminProps) {
  ...
  const saveUser = async () => {
    ...
    if (creating && (!String(editor.email || '').trim() || String(editor.password || '').length < 8)) { setNote('失敗：請填寫電子郵件，初始密碼至少 8 個字元'); return; }
    const selectedRole = String(editor.rbac_role || 'reporter');
    if (!['unit_supervisor', 'sysadmin'].includes(selectedRole) && !editor.supervisor_id) { setNote('失敗：一般人員必須指定直屬課室主管'); return; }
  ...
  return <AppShell profile={profile} title={module.title}>
    <AdminHeader module={module} busy={busy} note={note} onReload={load} action={<><span>待審申請 {pendingApplications.length} 筆</span><button className="primary-btn compact" onClick={() => setEditor({ rbac_role: 'reporter', status: 'active' })}>＋ 新增帳號</button></>}/>

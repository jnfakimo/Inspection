...
## 工程判斷與操作正確性（2026-09-14 使用者指定）

- 先取得證據再下判斷，不以猜測的主機、磁碟、目錄、服務名稱、連接埠、版本或相依條件產生操作指令。提供指令前，應以可用工具核對目標環境、`Test-Path`、程式版本、權限與服務狀態；無法核對的內容必須明確標示，不能包裝成已確認事實。
- 禁止把使用者當成試錯工具。不得連續要求使用者嘗試不同路徑、改幾個參數或反覆貼結果來探索環境；代理應先完成日誌、原始碼、遠端狀態與可存取設備的盤點，再提出一個有依據的處理方案。
- 必須由使用者在特定設備執行時，只提供一次整合好的最少步驟。指令使用完整絕對路徑或自包含的固定版本下載流程，包含前置檢查、SHA-256／版本驗證、遇錯即停及清楚的成功條件，不依賴目前工作目錄、殘留變數或 Markdown 跳脫字元。
- 修正方案須盡量可重複執行且不破壞既有資料；資料庫、部署與維修腳本必須在錯誤時回傳失敗，不可因子程序結束碼或輸出判讀缺陷誤報成功。
- 若現有證據不足以安全執行，應停止變更並精確說明缺少的唯一資訊或權限；不得用更多未驗證嘗試取代工程判斷。

...
- **Adding a page**: give it the shared navbar/topbar, the Supabase init block, and
  ...
  clock.
- **Shared header actions**: V2 一般內容頁（共 13 大系統的 65 個子系統）頂列只保留三個
...
- 13 大系統、65 個子系統的一般內容頁一律由 `AppShell` 自動插入
...
- **駐衛警系統入口（2026-08-27 使用者指定）**：`/v2/systems/guardpatrol/` 使用共用
  ...
  禁止水平溢出。
- 新增／修改系統子頁後必須執行 `npm run test:page-headings`；此檢查固定盤點 13／65、
  正式 Logo、標題 token，以及 60 個一般頁首與 4 個全螢幕頁首的覆蓋關係。

- **65 個子系統圖卡（2026-09-10 更新）**：一般 `.module-grid` 與交接／駐衛警入口

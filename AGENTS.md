...
→ `equipment_lifecycle.sql` → `patrol_shifts.sql` → `checkin_logs.sql` → `system_access_seed.sql`
→ `audit_login_events.sql` → `rls_hardening.sql` → `rls_hardening_login_fix.sql` → `permanent_data_protection.sql`.

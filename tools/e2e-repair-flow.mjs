...
async function createBootstrapUsers() {
  ...
  const email = `p0-e2e-${suffix}@example.invalid`;
  ...
  const sysadmin = (await rest('/users', {
    key: SERVICE_KEY,
    token: SERVICE_KEY,
    method: 'POST',
    ...
    body: {
      auth_id: authUser.id,
      name: 'P0 E2E 系統測試',
      username: `p0_e2e_${suffix.replaceAll('-', '_')}`,
      email,
      role: 'admin',
      rbac_role: 'sysadmin',
      status: 'active',
  ...
  const technician = (await rest('/users', {
    key: SERVICE_KEY,
    token: SERVICE_KEY,
    method: 'POST',
    prefer: 'return=representation',
    body: {
      name: 'P0 E2E 維修測試',
      username: `p0_tech_${suffix.replaceAll('-', '_')}`,
      role: 'maintenance',
      rbac_role: 'technician',
      status: 'active',
      hidden: true,
      supervisor_id: sysadmin.user_id,
      permissions: {},
    },
  }))[0];
  return { email, password, authUserId: authUser.id, sysadmin, technician };
}
...
async function loadExistingTestUser() {
  ...
  if (!['sysadmin', 'admin'].includes(String(sysadmin.rbac_role || sysadmin.role))) throw new Error('P0 E2E 測試帳號必須具備系統管理員權限');
  ...
  return { email, password, authUserId: authUser.id, sysadmin, technician: technicians[0] };
}
...
let requestId = '';
let orderId = '';
const result = { bootstrap: BOOTSTRAP, stages: [], negativeGuard: false };

try {
  const token = await signIn(fixture.email, fixture.password);
  const created = (await rest('/repair_requests', {
    ...
    body: {
      ...
      status: 'pending',
      hidden: true,
      created_by: fixture.sysadmin.user_id,
    },
  }))[0];
  ...
  const stages = [
    ['dispatch', { technician: fixture.technician.user_id, work_content: 'P0 端到端派工測試' }, 'assigned'],
    ['engineer_accept', {}, 'accepted'],
    ['engineer_start', {}, 'in_progress'],
    ['engineer_complete', { fault_cause: '測試故障原因', handle_method: '測試處理方式', labor_hours: 1, parts_used: '測試零件', note: 'P0 E2E' }, 'pending_review'],
  ];
  for (const [action, payload, expected] of stages) {
    await edgeAction(token, 'workorder_workflow', { request_id: requestId, workflow_action: action, payload });
    const state = await readFlow(token, requestId);
  ...
  try {
    await edgeAction(token, 'workorder_workflow', { request_id: requestId, workflow_action: 'supervisor_accept', payload: {} });
  ...
  await edgeAction(token, 'workorder_workflow', { request_id: requestId, workflow_action: 'reporter_accept', payload: {} });
  result.stages.push({ action: 'reporter_accept', status: (await readFlow(token, requestId)).request?.status });
  assert(result.stages.at(-1).status === 'completed', '報修人驗收後案件應為 completed');

  await edgeAction(token, 'workorder_workflow', { request_id: requestId, workflow_action: 'supervisor_accept', payload: {} });
  const finalState = await readFlow(token, requestId);
  ...
  assert(finalState.order?.assignee_id === fixture.technician.user_id, '工單指派技師不一致');
  ...
  if (BOOTSTRAP) {
    // 先停用下屬技師，再停用主管；階層觸發器要求啟用中的人員
    // 不能指向已停用的直屬主管。
    await rest(`/users?user_id=eq.${fixture.technician.user_id}`, {
      key: SERVICE_KEY,
      token: SERVICE_KEY,
      method: 'PATCH',
      body: { status: 'inactive', hidden: true },
    }).catch(() => undefined);
    await rest(`/users?user_id=eq.${fixture.sysadmin.user_id}`, {
      key: SERVICE_KEY,
      token: SERVICE_KEY,
      method: 'PATCH',
      body: { status: 'inactive', hidden: true },

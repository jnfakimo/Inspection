...
type UserRow = { user_id: string; name: string; dept_id: string | null; role: string | null; rbac_role: string | null; };
type DeptRow = { dept_id: string; parent_id: string | null; name: string; };
...
Deno.serve(async req => {
  ...
  try {
    ...
    const [documentsResult, departmentsResult, usersResult] = await Promise.all([
      ...
      db.from('departments').select('dept_id,parent_id,name').eq('status', 'active'),
      db.from('users').select('user_id,name,dept_id,role,rbac_role').eq('status', 'active'),
    ]);

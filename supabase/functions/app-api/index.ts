...
export async function handleAppApiRequest(req: Request) {
  ...
  try {
    ...
    // 公務車 API 依業務拆到 handlers/vehicle.ts，降低多人同時修改 index.ts 的衝突。
    ...
    const mechanicalHandoverResponse = await handleMechanicalHandoverAction(action, { req, body, profile, admin, userDb, reply, can, canModule, isAdmin, isSysadmin });
    if (mechanicalHandoverResponse) return mechanicalHandoverResponse;
...
if (equipment.error) throw equipment.error;
    ...
    // SYS-08 會議室預約 API 依業務拆到 handlers/meeting.ts；錯誤一樣由最外層 catch 統一處理。
    const meetingResponse = await handleMeetingAction(action, { req, body, profile, admin, userDb, reply, can, canModule, isAdmin, isSysadmin });

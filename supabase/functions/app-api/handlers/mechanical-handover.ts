...
export async function handleMechanicalHandoverAction(action: string, ctx: AppApiContext): Promise<Response | null> {
  ...
  const { req, body, profile, admin, userDb, reply, can, canModule, isSysadmin } = ctx;
  if (!can('handover') || !canModule('handover', 'mechanical')) {
    return reply(req, { ok: false, message: '目前帳號未開放機電課交接簿' }, 403);
  }
  const market = await authorizeHandoverMarket(admin, profile.user_id, 'mechanical', body.market_code, isSysadmin);
  if (!market) return reply(req, { ok: false, message: '目前帳號未開放所選市場的機電課交接簿' }, 403);
  if (action === 'mechanical_handover_receivers') {
    const { data, error } = await userDb.rpc('mechanical_market_receivers', { p_market: market });
  ...
  if (action === 'mechanical_handover_day') {
    const { data, error } = await userDb.rpc('mechanical_market_day', { p_market: market, p_date: date });
    if (error) throw error;
    return reply(req, { ok: true, data });
  }
  ...
  const { data, error } = await userDb.rpc('mechanical_market_action', {
    p_market: market, p_action: operation, p_date: date, p_shift: shift, p_receiver: receiver || null, p_revision: revision,
  });

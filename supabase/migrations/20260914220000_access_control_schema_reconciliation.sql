...
-- Reconcile self-hosted databases that received application releases after
-- 2026-09-11 without the matching granular-access migrations.  This migration
-- contains only the final four-layer access-control schema; it deliberately
-- does not replay unrelated handover or business tables.
...
revoke all on public.user_handover_module_access,public.user_system_access,public.user_module_access,public.role_module_access from anon;
revoke insert,update,delete on public.user_handover_module_access,public.user_system_access,public.user_module_access,public.role_module_access from authenticated;
grant select on public.user_handover_module_access,public.user_system_access,public.user_module_access,public.role_module_access to authenticated;
grant select,insert,update on public.user_handover_module_access,public.user_system_access,public.user_module_access,public.role_module_access to service_role;

drop policy if exists user_handover_module_access_admin_read on public.user_handover_module_access;
create policy user_handover_module_access_admin_read on public.user_handover_module_access for select to authenticated
...
-- Preserve the legacy explicit handover allow-list.  All other handover
-- modules become explicit deny for active non-admin users, matching the
-- original 2026-09-11 migration and preventing accidental privilege growth.
...
comment on table public.user_handover_module_access is 'Legacy per-user handover module allow-list retained for compatibility';
comment on table public.user_system_access is 'Per-user system access override: inherit, allow, or deny';
comment on table public.user_module_access is 'Per-user module access override; system deny always wins';
comment on table public.role_module_access is 'Per-role module template; personal overrides take precedence';

...
create or replace function purge_expired_vehicle_location_points(p_before timestamptz default now()-interval '90 days')
returns bigint
language plpgsql
security definer
set search_path=public
...
begin
  if auth.role() <> 'service_role' then
    raise exception '只有系統排程可以執行定位資料保存政策';
  end if;
  delete from vehicle_location_points where recorded_at<p_before;
  get diagnostics removed=row_count;
  return removed;
end;
$$;
revoke all on function purge_expired_vehicle_location_points(timestamptz) from public,anon,authenticated;
grant execute on function purge_expired_vehicle_location_points(timestamptz) to service_role;
...
begin
  ...
  if v_existing_identifier is not null
     ...
     and not (public.is_admin() or public.has_app_permission('dispatch')) then
...
create policy vehicle_tracking_devices_manage_insert on vehicle_tracking_devices for insert to authenticated
  with check (public.has_system_access('sys_vehicletracking') and (public.is_admin() or public.has_app_permission('dispatch')));
create policy vehicle_tracking_devices_manage_update on vehicle_tracking_devices for update to authenticated
  using (public.has_system_access('sys_vehicletracking') and (public.is_admin() or public.has_app_permission('dispatch')))
  with check (public.has_system_access('sys_vehicletracking') and (public.is_admin() or public.has_app_permission('dispatch')));
...
create policy vehicle_tracking_sessions_start on vehicle_tracking_sessions for insert to authenticated
  with check (public.has_system_access('sys_vehicletracking') and
    (driver_id=public.active_user_id() or public.is_admin() or public.has_app_permission('dispatch')));
...
select role_id,'sys_vehicletracking',true from roles
...
begin
  if to_regprocedure('public.reject_physical_data_removal()') is not null then
    foreach table_name in array array[
      'vehicle_tracking_devices','vehicle_tracking_sessions','vehicle_location_daily_summaries',

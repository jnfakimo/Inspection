-- 公務車派車申請與行車紀錄；完整可重跑版本另見 system/sql/vehicle_dispatch.sql。
begin;

create sequence if not exists vehicle_dispatch_seq;
create or replace function gen_vehicle_dispatch_no() returns text language sql as $$
...
create table if not exists vehicle_dispatch_requests (
  ...
  request_no text not null unique default gen_vehicle_dispatch_no(),
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint vehicle_dispatch_status_check check
    (status in ('draft','pending_approval','returned','approved','assigned','completed','cancelled')),
  constraint vehicle_dispatch_passenger_check check
    (passenger_count > 0 and (actual_passenger_count is null or actual_passenger_count >= 0)),
  constraint vehicle_dispatch_time_check check
    (planned_return_time > planned_departure_time and
      (actual_departure_at is null or actual_return_at is null or actual_return_at >= actual_departure_at)),
  constraint vehicle_dispatch_mileage_check check
    ((odometer_start is null or odometer_start >= 0) and
...
create or replace function touch_vehicle_updated_at() returns trigger language plpgsql as $$
begin new.updated_at=now(); return new; end;
$$;
drop trigger if exists trg_official_vehicles_updated_at on official_vehicles;
create trigger trg_official_vehicles_updated_at before update on official_vehicles
...
create trigger trg_vehicle_dispatch_updated_at before update on vehicle_dispatch_requests
  for each row execute function touch_vehicle_updated_at();
...
create policy "vehicle_authenticated" on official_vehicles for all to authenticated using (true) with check (true);
create policy "vehicle_dispatch_authenticated" on vehicle_dispatch_requests for all to authenticated using (true) with check (true);
create policy "vehicle_dispatch_logs_authenticated" on vehicle_dispatch_logs for all to authenticated using (true) with check (true);

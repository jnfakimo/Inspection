...
## 2026-09-11 可留白下拉選單與人員清單規則

- 允許空值的資料下拉選單，第一列必須是真正空白值，不得自動代選第一筆或以提示文字冒充空值；狀態、必要動作及表格篩選不適用。
- 人員指派、排班及簽名清單必須使用共用有效人員過濾，排除停用、離職與去識別化帳號，不能只依 `status='active'` 判定。
- 可維護主檔採新增、修改、停用及恢復；已被歷史紀錄引用的選項不做實體刪除。

## 2026-09-13 AntiGravity ✕ OpenCode CI 雙層協同省 Token 工作流

- **架構分工**：
  - **AntiGravity (架構師/審查者)**：負責需求分析、定位修改位置、產出結構化【OpenCode 任務執行卡】Prompt，不輸出冗長完整檔案以極致節省 Token。
  - **OpenCode / CI (執行者)**：依據結構化 Prompt 在本地端或輕量環境中精確修改代碼與執行初步測試。
  - **AntiGravity (驗收/守門員)**：接收 OpenCode 成果後，執行 `git diff` 審查、ISO 27001 資安掃描、全套自動化測試驗收，最後安全 Commit & Push。
- **標準手冊**：詳見 `docs/AGENT_COLLABORATION_WORKFLOW.md`。
